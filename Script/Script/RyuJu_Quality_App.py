...
def check_password():
    ...
    if st.button("ログイン", type="primary"):
        # --- 認証ロジック ---
        # TODO: 本番運用時はここでDB照合やハッシュ化を行うが、現在は簡易チェックとする
...
def generate_prompt_template(facts):
    ...
    【強化点】論理的連鎖（Chain of Logic）に加え、逆検証（Reverse Verification）を出力させる
    """
    prompt = f"""
# 不具合原因分析要請（論理連鎖・即時逆検証型）

あなたは製造業における熟練の品質保証エンジニアであり、論理的思考（ロジカルシンキング）のプロフェッショナルです。
提供された事実に基づき、論理の飛躍を排除した「なぜなぜ分析」を行い、**各分析の直後に必ず「論理の逆検証」を表示**して対策を立案してください。
...
### Step 2: なぜなぜ分析（発生要因）と「逆検証」
不具合が「発生したメカニズム」について深掘りします。
以下の**【連鎖ルール】**を厳守し、根本原因（真因）に到達するまで分析を行ってください。

**【厳守ルール：連鎖の法則】**
1. **「なぜ？」の対象を固定する**: 前の段階の「答え（Ans）」が、次の段階の「問い（Why）」の主語にならなければなりません。
2. **逆検証の可視化**: 分析チェーンの後に、必ず「原因」から「結果」へ「だから（Therefore）」でつないだ文章を作成・表示してください。
3. **ID付与**: 全ての末端の要因（根本原因）には、必ず一意のID（例: 発-1, 発-2...）を付与してください。

*出力イメージ:*
- **[事象]**: 異品が混入した
  - **Why1**: なぜ異品が混入したのか？
    - **Ans1**: ラインサイドに類似品が置かれていたから
      - **Why2**: なぜラインサイドに類似品が置かれていたのか？
        - **Ans2**: 前のロットの部品回収が漏れていたから
           - **Why3**: なぜ回収が漏れていたのか？
             - **Ans3 (根本原因)**: チェックシートに回収項目の記載がなかったから **(ID: 発-1)**
...
> **だから**、[Ans1] という状態になり、
> **だから**、[事象] が発生した。
> *(判定: 論理成立 / 修正が必要ならここに記述)*

### Step 3: なぜなぜ分析（流出要因）と「逆検証」
「なぜ工程内で発見・除去できずに後工程（または客先）へ流れたか？」を分析してください。
ここでもStep 2と同様に**【連鎖ルール】と【逆検証】**を適用してください。
...
- **[事象]**: 検査で見逃した
  - **Why1**: ...
    - **Ans1**: ...
      - **Why2**: ...
        - **Ans2**: 手元に最新の見本がなかったから **(ID: 流-1)**

> **🔄 逆検証 (Reverse Check)**
> [根本原因: Ans2] だった。**だから** ... **だから**、[事象] が流出した。

### Step 4: 対策提案（紐づけ必須）
特定された真因に対し、**「発生対策」と「流出対策」を明確に分けて**提案してください。
精神論（気をつける）は避け、物理的な対策（ポカヨケ・仕組み化）を優先してください。
...
def main():
    ...
    with col_title:
        # st.title ではなく st.markdown を使い、## でサイズ調整します
        st.markdown("## 🔍 品質不具合対策論理分析支援ツール「🐉 龍樹 RyuJu -Quality-」")
...
"""
        ...
        if not input_what or not input_how:
            ...
            st.info("下のテキストエリア内を全選択（Ctrl+A）してコピー（Ctrl+C）。ChatGPTやGeminiに貼り付けてください")
            ...
            **このプロンプトの5つの特徴:**
            ...
            2. **連鎖ルールと逆検証:** 論理の飛躍を防ぎ、強固な因果関係を構築。
            3. **逆検証の可視化:** 各分析の後に「だから〜になった」を表示し、納得感を高める。
            4. **ツリー構造 & ID付与:** 原因の分岐を明確化し、IDで管理。
            5. **対策の紐づけ & 区分:** IDを用いた確実な対策立案と、暫定/恒久の峻別。
            """)

if __name__ == "__main__":
    # パスワード認証チェック
    # ログインしていない場合はログイン画面のみ表示し、main()は実行しない
    if check_password():
        main()
