...
提供された事実に基づき、不安全行動と不安全状態を特定し、論理的な「なぜなぜ分析」を行ってください。
また、ヒヤリハット事例の場合は、ハインリッヒの法則に基づき「一歩間違えればどのような重大災害に繋がっていたか」という潜在的リスクも考慮してください。
...
### Step 1: 要因分類とリスク推論
入力された事実記述から、以下の4要素に関連する要素を抽出・整理してください。
ヒヤリハットの場合、この事象が「どのような重大災害に繋がる可能性があったか」をまず推定してください。
...
**末端の要因（根本原因）には、必ず一意のID（例: 人-1, 人-2...）を付与してください。**

### Step 3: なぜなぜ分析（物的要因の深掘り）
「なぜ設備に不備があったか？」「なぜ管理で見抜けなかったか？」を分析してください。
**末端の要因（根本原因）には、必ず一意のID（例: 物-1, 物-2...）を付与してください。**
...
※対策は「暫定（即処置）」と「恒久（物理的・仕組みの改善）」に分けて記述。特に「個人の注意」に頼らない物理的な対策を優先してください。
...
def main():
    ...
    with col_title:
        # メインタイトルの十字もグリーンにする
        st.markdown('<h1><span style="color: #008000;">✙</span> 労災・ヒヤリハット論理分析支援ツール「龍樹 RyuJu -Safety-」</h1>', unsafe_allow_html=True)
...
・【ヒヤリ】コンベアの詰まりを直そうとして、機械を止めずに手を入れかけた。
・【ヒヤリ】床の油に足を取られ、転倒しそうになった。
...
"""
        ...
        if not input_how:
            ...
            st.markdown("""
            **安全・ヒヤリ分析プロンプトの特徴:**
            1. **ハインリッヒの視点:** ヒヤリハットから将来の重大災害を予測し、未然防止を促します。
            2. **人的・物的要因の分離:** 行動、管理、設備不備、設備管理の4軸で真因を追究させます。
            3. **ポカヨケ優先:** 個人の注意喚起ではなく、物理的に起きない仕組みをAIに考えさせます。
            """)
